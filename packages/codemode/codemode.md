...
This is the living design and status document for `@opm/codemode` and its existing V2 OPM adapter.
...
`packages/opm/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
