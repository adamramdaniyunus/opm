...
- The server still lives in `packages/opm`.
...
Until enough shared service code lives outside `packages/opm`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/opm`

It should not import `packages/opm` services while `packages/opm`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/opm` implementation details.
