...
Delete `packages/opm/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/opm/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/opm/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/opm`
- TUI plugin loader tests in `packages/opm` if runtime plugin API wiring changed
