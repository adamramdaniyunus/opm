...
> current private `@opm/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@opm/ai`
...
- Secondary audience: OPM and other durable agent runtimes
...
import { LLM } from "@opm/ai"
import { OpenAI } from "@opm/ai/providers/openai"
...
OPM and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@opm/ai/promise"
import { OpenAI } from "@opm/ai/providers/openai"
...
```ts
import { LLMSchema } from "@opm/ai/schema"
...
```ts
import { Provider, Protocol } from "@opm/ai/provider"
...
@opm/ai/providers/openai
@opm/ai/providers/anthropic
@opm/ai/providers/google
...
```
...
OPM should migrate to `generateTurn` / `streamTurn`, preserving its durable
