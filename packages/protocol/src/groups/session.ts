...
import { PromptInput } from "@opm/schema/prompt-input"
import { Session } from "@opm/schema/session"
import { Project } from "@opm/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@opm/schema/schema"
...
import { SessionEvent } from "@opm/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this OPM process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this OPM process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
